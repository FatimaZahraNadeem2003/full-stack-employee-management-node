...
export const HandleHRSignup = async (req, res) => {
    try {
        const { firstname, lastname, 
            email, password, contactnumber, name, description, OrganizationURL, OrganizationMail } = req.body

        if (!name || !description || !OrganizationURL || !OrganizationMail) {
            console.log('Missing organization fields');
            return res.status(400).json({ success: false, message: "All organization fields are required: name, description, OrganizationURL, OrganizationMail", type: "signup" });
        }

        if (!firstname || !lastname || !email || !password || !contactnumber) {
            console.log('Missing personal fields');
            return res.status(400).json({ success: false, message: "All personal fields are required: firstname, lastname, email, password, contactnumber", type: "signup" });
        }

        console.log('Checking for existing organization and HR...');
        ...
        if (!organization && !HR) {
            ...
            console.log('Organization created with ID:', newOrganization._id.toString());

            const hashedpassword = await bcrypt.hash(password, 10)
            const verificationcode = GenerateVerificationToken(6)

            console.log('Creating new HR...');
            ...
            console.log('HR created with ID:', newHR._id.toString());

            newOrganization.HRs.push(newHR._id)
            await newOrganization.save()

            GenerateJwtTokenAndSetCookiesHR(res, newHR._id, newHR.role, newOrganization._id)
            SendVerificationEmail(email, verificationcode).catch(err => console.error("Failed to send verification email:", err))
        ...
        if (organization && !HR) {
            console.log('Using existing organization, creating new HR...');
            const hashedpassword = await bcrypt.hash(password, 10)
            const verificationcode = GenerateVerificationToken(6)
        ...
        if (error.code === 11000) {
            ...
            return res.status(400).json({ success: false, message: errorMessage, type: "signup" });
        }
        
        return res.status(500).json({ success: false, message: error.message || 'An error occurred during signup', type: "signup" })
    }
}
...
export const HandleHRLogin = async (req, res) => {
    ...
    try {
        ...
        await HR.save()
        return res.status(200).json({ success: true, message: "HR Login Successfull", type: "HRLogin", HRID: HR._id })
    }
    catch (error) {
...
export const HandleHRCheck = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, message: "HR Already Logged In", type: "checkHR", HRID: HR._id })
    } catch (error) {
